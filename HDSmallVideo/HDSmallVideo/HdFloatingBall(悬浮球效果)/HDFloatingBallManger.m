...
@interface HDFloatingBallManger()<UIGestureRecognizerDelegate,UINavigationControllerDelegate>
...
/** 右下角1/4圆 */
@property (nonatomic, strong) OneFourthBallView *cancelFloatView;
/** 栈顶即将要pop的控制器 */
...
+(instancetype)shared{
    ...
    dispatch_once(&onceToken, ^{
        ...
        manger.monitorVcClasses = [NSMutableArray array];
        [UIViewController currentNavigationController].interactivePopGestureRecognizer.delegate = manger;
        [UIViewController currentNavigationController].delegate = manger;
        [[NSNotificationCenter defaultCenter] addObserver:manger selector:@selector(p_animationWillBegin) name:AnimationWillBeginKey object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:manger selector:@selector(p_animationWillEnd) name:AnimationWillEndKey object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:manger selector:@selector(p_animationDidEnd:) name:AnimationDidEndKey object:nil];
...
-(id<UIViewControllerAnimatedTransitioning>)floatingBallForOperation:(UINavigationControllerOperation)operation fromViewController:(UIViewController *)fromVC toViewController:(UIViewController *)toVC{
    ...
    if (operation == UINavigationControllerOperationPush) {
        ...
        if (fromVC == self.lastPopViewController) {
            ...
            NSNumber *popWithPanGesNumber = objc_getAssociatedObject([UIViewController currentNavigationController], &kPopWithPanGes);
...
-(void)handleNavigationTransition:(UIScreenEdgePanGestureRecognizer *)gestureRecognizer{
    ...
    if (gestureRecognizer.state == UIGestureRecognizerStateBegan) {
        if (notShowFloatContent && self.showStyle == FloatShowStyleShow && self.floatViewController != self.lastPopViewController) {
            [self.cancelFloatView isShowingOneFourthView:NO];
        }
        objc_setAssociatedObject([UIViewController currentNavigationController], &kPopWithPanGes, [NSNumber numberWithBool:YES], OBJC_ASSOCIATION_ASSIGN);
        ...
        objc_setAssociatedObject([UIViewController currentNavigationController], &kPopWithPanGes, [NSNumber numberWithBool:NO], OBJC_ASSOCIATION_ASSIGN);
        if (notShowFloatContent) {
...
- (void)tapFloatView:(UITapGestureRecognizer *)ges
{
    ...
    if ([[[UIViewController currentNavigationController].childViewControllers lastObject] isEqual:self.floatView]) {
        return;
    }
    [self.cancelFloatView isShowingOneFourthView:YES];
    [[UIViewController currentNavigationController] pushViewController:self.floatViewController animated:YES];
}
...
#pragma mark - NSNotificationCenter 监听方法
...
{
    if ([UIViewController currentNavigationController].viewControllers.count > 1) {
        if ([self.monitorVcClasses containsObject:NSStringFromClass([[UIViewController currentViewController] class])]) {
            [[UIApplication sharedApplication].keyWindow addSubview:self.floatView];
            self.lastPopViewController = [UIViewController currentViewController];
        }
        else {
            self.lastPopViewController = nil;
        }
...
- (OneFourthBallView *)cancelFloatView
{
    if (!_cancelFloatView) {
        _cancelFloatView = [[OneFourthBallView alloc] initWithFrame:CGRectMake(FloatScreenWidth, FloatScreenHeight , RoundViewRadius, RoundViewRadius)];
